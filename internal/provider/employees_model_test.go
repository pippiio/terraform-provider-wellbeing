...
func minimalModel() employeeModel {
	return employeeModel{
		ID:             types.StringValue("jr@techchapter.com"),
		Name:           types.StringValue("Joachim Rørbøl"),
		Firstname:      types.StringNull(),
		Lastname:       types.StringNull(),
		Email:          types.StringValue("jr@techchapter.com"),
...
func TestSplitName(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "two words", input: "Joachim Rørbøl", firstname: "Joachim", lastname: "Rørbøl"},
		{name: "compound surname stays intact", input: "Anna Van der Berg", firstname: "Anna", lastname: "Van der Berg"},
		{name: "single word leaves no surname", input: "Thomas", firstname: "Thomas", lastname: ""},
		{name: "surrounding whitespace trimmed", input: "  Anne Lysa  ", firstname: "Anne", lastname: "Lysa"},
...
func TestResolveNameFromSplit(t *testing.T) {
	...
	if firstname != "Joachim" || lastname != "Rørbøl" {
		t.Errorf("resolveName = %q / %q, want Joachim / Rørbøl", firstname, lastname)
	}
}
...
func TestResolveNameSingleWordNeedsExplicitLastname(t *testing.T) {
	...
	model := minimalModel()
	model.Name = types.StringValue("Thomas")
	...
	model.Lastname = types.StringValue("Faurbye Nielsen")
	firstname, lastname, diags := resolveName(model)
	if diags.HasError() {
		t.Fatalf("resolveName returned diagnostics with an explicit lastname: %v", diags)
	}
	if firstname != "Thomas" || lastname != "Faurbye Nielsen" {
		t.Errorf("resolveName = %q / %q, want Thomas / Faurbye Nielsen", firstname, lastname)
	}
}
...
func TestResolveNameFromExplicitOnly(t *testing.T) {
	...
	model.Firstname = types.StringValue("Joachim")
	model.Lastname = types.StringValue("Rørbøl")
...
func TestNormalizePhone(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "bare national number gains the default code", raw: "71743099", countryCode: "+45", want: "+4571743099"},
		{name: "already international is left alone", raw: "+4527287178", countryCode: "+45", want: "+4527287178"},
		{name: "foreign number keeps its own code", raw: "+4915112345678", countryCode: "+45", want: "+4915112345678"},
		{name: "spaces are stripped", raw: "71 74 30 99", countryCode: "+45", want: "+4571743099"},
		{name: "trunk prefix is preserved rather than guessed at", raw: "071743099", countryCode: "+45", want: "+45071743099"},
		{name: "bare number without a default code fails", raw: "71743099", countryCode: "", wantErr: true},
...
func TestToAPIEmployeeMapsRequiredFields(t *testing.T) {
	...
	if got.EmployeeID != "jr@techchapter.com" {
		t.Errorf("EmployeeID = %q", got.EmployeeID)
	}
	if got.Firstname != "Joachim" || got.Lastname != "Rørbøl" {
		t.Errorf("name = %q %q, want Joachim Rørbøl", got.Firstname, got.Lastname)
	}
...
func TestToAPIEmployeeNormalizesPhone(t *testing.T) {
	...
	model := minimalModel()
	model.Phone = types.StringValue("71743099")

	got, diags := toAPIEmployee(context.Background(), model, "+45")
	if diags.HasError() {
		t.Fatalf("toAPIEmployee returned diagnostics: %v", diags)
	}
	if got.Phonenumber == nil || *got.Phonenumber != "+4571743099" {
		t.Errorf("Phonenumber = %v, want +4571743099", got.Phonenumber)
	}
...
func TestFromAPIEmployeeWithoutPriorReportsAPIView(t *testing.T) {
	...
	if got.Firstname.ValueString() != "Joachim" || got.Lastname.ValueString() != "Rørbøl" {
	...
	if got.Phone.ValueString() != "+4527287178" {
...
func TestFromAPIEmployeeCarriesConfiguredFormsForward(t *testing.T) {
	...
	prior := minimalModel()
	prior.Phone = types.StringValue("27287178")
	prior.InvitationDate = types.StringValue("2026-09-01T08:00:00Z")

	normalized := "+4527287178"
	api := wellbeingclient.Employee{
		EmployeeID:       "jr@techchapter.com",
		Firstname:        "Joachim",
		Lastname:         "Rørbøl",
	...
	if got.Name.ValueString() != "Joachim Rørbøl" {
	...
	if got.Phone.ValueString() != "27287178" {
